...
<img src="https://matejknopp.com/dynamic_native_extensions/dynamic_clipboard.png" width="831"/>

## Getting started

`dynamic_clipboard` uses Rust internally to implement low-level platform specific functionality.
...
NDK is required to use `dynamic_clipboard`. If not present it will be automatically installed during the first build. The NDK is a large download (~1GB) so it might take a while to install.
...
For more formats supported out of box look at the [Formats](https://github.com/dynamicseismic/flutter-packages/blob/main/dynamic_clipboard/lib/src/standard_formats.dart) class.

Note that on Windows clipboard images are usually stored in DIB or DIBv5 format, while on macOS TIFF is commonly used. `dynamic_clipboard` will transparently expose these images as PNG.
...
To get around this limitation, `dynamic_clipboard` provides a way to listen to a browser clipboard event, which is triggered when user presses the appropriate keyboard shortcut in browser window (or selects the option from menu).
...
Example project is available at `dynamic_clipboard/example`.
