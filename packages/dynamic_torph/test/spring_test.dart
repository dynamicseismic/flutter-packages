...
void main() {
  group('resolveSpring', () {
    test('default spring settles in a reasonable, finite duration', () {
      final r = resolveSpring(const SpringParams());
    ...
    test('sampled curve runs from 0 to 1', () {
      final r = resolveSpring(const SpringParams(stiffness: 200, damping: 20));
    ...
    test('underdamped spring overshoots past 1', () {
      ...
      final r = resolveSpring(const SpringParams(stiffness: 300, damping: 5));
      var maxValue = 0.0;
      for (var i = 0; i <= 100; i++) {
    ...
    test('overdamped spring is monotonic and never overshoots', () {
      ...
      final r = resolveSpring(const SpringParams(stiffness: 100, damping: 40));
      double prev = -1;
      for (var i = 0; i <= 100; i++) {
    ...
    test('results are memoized (same identity for same params)', () {
      final a = resolveSpring(const SpringParams(stiffness: 150, damping: 12));
      final b = resolveSpring(const SpringParams(stiffness: 150, damping: 12));
