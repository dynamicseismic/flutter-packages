...
import 'package:dynamic_morphing_icons_example/main.dart';
...
void main() {
  testWidgets('tapping the hero cycles to the next icon', (tester) async {
    ...
    expect(find.text(DynamicMorphIcons.all.first.name), findsOneWidget);

    // Tap the hero (its semantic label) and let the morph settle.
    await tester.tap(find.bySemanticsLabel('Cycle to next icon'));
    await tester.pumpAndSettle();

    expect(find.text(DynamicMorphIcons.all[1].name), findsOneWidget);
  });
}
