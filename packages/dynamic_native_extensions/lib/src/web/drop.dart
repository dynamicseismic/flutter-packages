...
Iterable<(List<String> formats, $DataReaderItemHandle? readerHandle)>
  ...
  // types.toDart yields List<JSString>; compare each as a Dart String so the
  // "Files" check actually matches (a Dart-String contains never would).
  final hasFiles = dataTransfer.types.toDart.any((e) => e.toDart == "Files");
...
class DropContextImpl extends DropContext {
  ...
  /// https://github.com/dynamicseismic/flutter-packages/issues/98
  web.EventTarget? _lastDragEnter;

  @visibleForTesting
  static const listenersProperty = 'dynamic_native_extensions_listeners';
