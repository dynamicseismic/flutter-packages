#import "DynamicNativeExtensionsPlugin.h"

#include <objc/runtime.h>

extern void dynamic_native_extensions_init(void);
extern bool dynamic_native_extensions_text_input_plugin_cut(void);
extern bool dynamic_native_extensions_text_input_plugin_copy(void);
extern bool dynamic_native_extensions_text_input_plugin_paste(void);
extern bool dynamic_native_extensions_text_input_plugin_select_all(void);

static void swizzleTextInputPlugin();

@implementation DynamicNativeExtensionsPlugin
...
- (void)cut_:(id)sender {
  if (!dynamic_native_extensions_text_input_plugin_cut()) {
    [self cut_:sender];
  }
}

- (void)copy_:(id)sender {
  if (!dynamic_native_extensions_text_input_plugin_copy()) {
    [self copy_:sender];
  }
}

- (void)paste_:(id)sender {
  if (!dynamic_native_extensions_text_input_plugin_paste()) {
    [self paste_:sender];
  }
}

- (void)selectAll_:(id)sender {
  if (!dynamic_native_extensions_text_input_plugin_select_all()) {
