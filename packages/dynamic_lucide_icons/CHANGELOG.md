...
- Declarative animation engine: per-part rotate / translate / scale / opacity /
  progressive path-draw, composable, with a const-friendly `SpringCurve`.
- Self-contained SVG path parser (`M/L/H/V/C/S/Q/T/A/Z`, absolute & relative) —
  no third-party runtime dependencies.
- `AnimatedLucideIcon` widget with hover, tap, and looping triggers, plus an
  `AnimatedLucideIconController` for programmatic control.
